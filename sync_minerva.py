...
功能：获得一个ipns的key
输入：name， ipns的name
返回：address， ipns的name和地址
"""
def create_ipns_key(name):
    res = client.key.gen(name, "rsa")
    address = res["Id"]
    return address
...
返回：ipns的地址
"""
def publish_ipns(name, address):
    res = client.name.publish(address, key=name)
    #print(res)
    ipns = res["Name"] #ipns地址
    return ipns
...
if __name__=="__main__":
    name = "maris2"
    #address = create_ipns_key(name)
    #print(address)
    address = "QmYUQee1mTUGjEMtk2ZBn8LdE1YVWueF3q5ec7WMgXsGfc"
    ipns = publish_ipns(name, address)
    print(ipns)
