...
def get_metadata_test():
    ...
    url = "http://0.0.0.0:8080/get_metadata?db_link={}&nft_id={}".format(
...
def insert_metadata_test():
    url = "http://0.0.0.0:8080/insert_metadata"
...
def search_metadata_test():
    ...
    url = "http://0.0.0.0:8080/search_metadata?db_link={}&field={}&word={}".format(
        urllib.parse.quote(db_link),
        field,
        word
    )
...
def upload_file_test():
    ...
    url = "http://0.0.0.0:8080/upload_file"
    header = {"Content-Type": "application/json"}
    data = {
        "file": base64_str
    }
